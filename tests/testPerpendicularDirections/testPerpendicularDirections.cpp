#include "../../include/Lattices/IntegerLattice.h"
#include <iostream>
#include <random>
...
int main() {
  ...
  Eigen::Matrix<IntScalarType, Eigen::Dynamic, Eigen::Dynamic> out(dim - 1,
                                                                   dim);
    ...
    for (int i = 1; i < dim; i++) {
      ...
      if (mat.row(0).dot(mat.row(i)) != 0)
        throw std::runtime_error(
            "Output directions are not perpendicular to the given direction\n");
    }
