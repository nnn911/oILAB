...
int main() {
  ...
  try {
    ...
    // temperature = 0.2;
    // resetEvery = 1000 - resets the Monte Carlo every 100 accepted states to
    // avoid getting stuck in a metastable state maxIterations = 20000 - total
    // number of Monte Carlo steps

    /* Standard Monte Carlo
    const auto& constraintsMesostateMap=
    ensemble.evolveMesoStates(0.3,1000,20000,"ms"); StandardMC<XTuplet>
    standardMC(2000.0);
    MonteCarlo<XTuplet,GbMesoState<3>,GbMesoStateEnsemble<3>,StandardMC<XTuplet>>
    mc(ensemble,standardMC);
    */
