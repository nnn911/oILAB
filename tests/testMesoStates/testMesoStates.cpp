...
int main() {
  ...
  try {
    ...
    std::cout << "Total number of training mesostates = "
              << mesostatesForTraining.size() << std::endl;
    ...
    energiesFile << "Mesostate" << std::setw(15) << "# CSL points"
                 << std::setw(15) << "Elastic energy" << std::endl;
    mesostateIndex = 0;
    // for(const auto& ms : builtMesostates) {
    int numberOfInteractingPlanes = 15;
    for (const auto &ms : mesostates) {
      energiesFile
          << mesostateIndex << std::setw(15)
          << ms.getLocalStateCount(numberOfInteractingPlanes).transpose()
          << std::setw(15) << ms.elasticEnergy() << std::endl;
      mesostateIndex++;
    }
    energiesFile.close();
