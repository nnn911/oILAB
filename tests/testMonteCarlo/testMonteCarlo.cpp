...
void runMonteCarlo(const double &a0, const double &temperature,
  ...
  misorientation angle VectorDimD gbNormal(-5,5,14);                        //
  Miller indices int heightScaling= 2; int periodScaling= 1; int axisScaling= 1;
  double bScaling= 1.4;
   */
...
std::pair<double,XTuplet> getLowestEnergyState(const std::string& filename) {
    ...
    double min_value = std::numeric_limits<double>::infinity(); // Initialize
...
// Check if this value is the lowest we have encountered
                if (current_value < min_value) {
                    ...
                    row_with_min_energy = row; // Store the entire row with the
minimum value
                }
            } catch (const std::invalid_argument &e) {
// Handle the case where conversion to double fails
                std::cerr << "Invalid number in the file: " << e.what() <<
std::endl;
            }
        }
    }
    ...
    if (!row_with_min_energy.empty()) {
        ...
        for (auto it = row_with_min_energy.begin(); it !=
std::prev(std::prev(row_with_min_energy.end())); ++it) {
  ...
  for (int i = 5; i <= 5; ++i) {
    ...
    for (int j = 0; j <= 0; ++j) {
      for (int k = 0; k < 100; ++k) {
        ...
        runMonteCarlo(a0Local, temperatureLocal, iterations,
                      "MC" + std::to_string(k) + "observablesT" +
                          std::to_string(temperatureLocal) + "_a" +
                          std::to_string(strain));
      }
    }
  }
  return 0;
