...
/**********************************************************************/
void RLLL::size_reduce(MatrixType& M, const int& k, const int& j)
{
...
/**********************************************************************/
RLLL::RLLL(const MatrixType& B0, const double& delta)
    : /* init */ B(B0),
      /* init */ U(Eigen::Matrix<long long int, Eigen::Dynamic, Eigen::Dynamic>::Identity(B0.cols(), B0.cols()))
{
    ...
    if(n < dim) {
        ...
        if(err > FLT_EPSILON) {
            Logger::debug() << "RLLL relative error= " << std::setprecision(15) << std::scientific << err << " > " << FLT_EPSILON;
            throw std::runtime_error("Relative error too large. RLLL failed.");
        }

        if(fabs(absDetU - 1.0) > FLT_EPSILON) {
            Logger::debug() << "|det(U)|= " << std::setprecision(15) << std::scientific << absDetU;
...
/**********************************************************************/
const typename RLLL::MatrixType& RLLL::reducedBasis() const { return B; }

/**********************************************************************/
const Eigen::Matrix<long long int, Eigen::Dynamic, Eigen::Dynamic>& RLLL::unimodularMatrix() const { return U; }
