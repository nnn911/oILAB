 ...
 */
template <typename Derived> class StaticID {
...
public:
  ...
  /**********************************************************************/
  static size_t nextID() { return count; }
  ...
  static void set_count(const size_t &newCount) {
    if (newCount < count) {
      throw std::runtime_error(
          "StaticID error: YOU ARE TRYING TO SET THE COUNTER TO A LOWER VALUE "
...
/* Static data members  *****************************/
template <typename Derived> size_t StaticID<Derived>::increment = 1;

template <typename Derived> size_t StaticID<Derived>::count = 0;

template <typename Derived> bool StaticID<Derived>::count_used = false;
