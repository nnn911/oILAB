...
template <int dim> class GbMesoState : public GbContinuum<dim> {
   ...
   * x^i)=\textbf u^i\f$ for the GB continuum model from the translation-shift
   * pairs defining the mesostate.
   * @param gb - grain boundary
   * @param bs - a deque of pairs (translation vector \p b, shift vector \p s).
   * \p b is a DSCL vector while \p s is expressed in Cartesian coordinates
   * @return A map between the DSCL coordinates \f$\underline{\textbf
   * x}^i_{\mathcal D}\f$ of \f$\textbf x^i\f$ and Cartesian coordinates of
   * \f$\textbf u^i\f$.
   */
  ...
  /*!
   * \brief Returns the Cartesian coordinates of the lattice vectors of the
   * mesostates's bicrystal in the form of a map that maps the DSCL integer
   * coordinates to the Cartesian coordinates.
   * @param bicrystalConfig - a vector of lattice vectors in the mesostates'
   * bicrystal.
   * @return A map between the DSCL coordinates of lattice vectors in
   * bicrystalConfig to their Cartesian coordinates.
   */
  static std::map<OrderedTuplet<dim + 1>, VectorDimD>
  bicrystalCoordsMap(const Gb<dim> &gb,
...
public:
   ...
   * contained in the box described by mesoStateCslVectors. They are used to
   * construct a function basis for the continuum solution.
   */
  const BicrystalLatticeVectors &bicrystalConfig;

  /*!
   * @param bs a deque of pairs (translation vector \f$\textbf b\f$, shift
   * vector \f$\textbf s\f$) that defines a mesostate. Translating lattice
   * \f$\mathcal A\f$ by \f$\textbf b\f$ results in a CSL shift of \f$\textbf
   * s\f$.
   */
  const std::deque<std::tuple<LatticeVector<dim>, VectorDimD, int>> bs;

  // ensure that b in bs pair belongs to the DSCL vectors and s belongs to the
  // box ensure that the lattice vectors in bicrystalConfig lie entirely in the
  // box
  ...
  densityEnergy(const std::string &lmpLocation,
                const std::string &potentialName, bool relax,
                const std::array<Eigen::Index, dim> &n =
                    std::array<Eigen::Index, dim>{}) const;
