...
LatticeDirection<dim> Lattice<dim>::latticeDirection(const VectorDimD& d, const double& tol) const
{
    ...
    if(crossNorm > tol) {
        ...
        Logger::debug() << "lattice direction=" << temp.cartesian().normalized().transpose();
        Logger::debug() << "cross product norm=" << std::setprecision(15) << std::scientific << crossNorm;
        Logger::debug() << "tolerance=" << std::setprecision(15) << std::scientific << tol;
...
ReciprocalLatticeDirection<dim> Lattice<dim>::reciprocalLatticeDirection(const VectorDimD& d, const double& tol) const
{
    ...
    if(crossNorm > tol) {
        Logger::debug() << "input direction=" << std::setprecision(15) << std::scientific << d.normalized().transpose();
        Logger::debug() << "reciprocal lattice direction=" << std::setprecision(15) << std::scientific
                                                                     ...
                                                                     const double& directionTol) const
{
    ...
    if((rld.cartesian() - d).squaredNorm() > magnitudeTol) {
        ...
        Logger::debug() << "d.norm()/ld.cartesian().norm()=" << d.norm() / ld.latticeVector().norm();
...
template<int dim>
RationalReciprocalLatticeDirection<dim> Lattice<dim>::rationalReciprocalLatticeDirection(
    const VectorDimD& d,
    const typename BestRationalApproximation::LongIntType& maxDen,
    const double& magnitudeTol,
    const double& directionTol) const
{
    ...
    if((rrld.cartesian() - d).squaredNorm() > magnitudeTol) {
        ...
        Logger::debug() << "reciprocal lattice direction=" << rld.cartesian().transpose();
        Logger::debug() << "rational=" << rat;
        Logger::debug() << "d.norm()/rld.cartesian().norm()=" << d.norm() / rld.reciprocalLatticeVector().norm();
                                                                           ...
                                                                           const bool& useRLLL) const
{
    ...
    if constexpr(dim > 2) {
        ...
        for(int i = 0; i < dim; ++i) outIntegerCoords.col(i) = out[i].latticeVector();
        Eigen::Matrix<IntScalarType, dim, dim - 1> planeParallelLatticeBasisIntegerCoords =
            outIntegerCoords.block(0, 1, dim, dim - 1) * rlll.unimodularMatrix();
        for(int i = 1; i < dim; ++i) out[i] = LatticeDirection<dim>(planeParallelLatticeBasisIntegerCoords.col(i - 1), *this);
    }
    /*
    if(dim>2) {
                                                                                                     ...
                                                                                                     const bool& useRLLL) const
{
    ...
    if constexpr(dim > 2) {
        ...
        MatrixDimI outIntegerCoords;
        for(int i = 0; i < dim; ++i) outIntegerCoords.col(i) = out[i].reciprocalLatticeVector();
...
template<int dim>
std::vector<typename Lattice<dim>::MatrixDimD> Lattice<dim>::generateCoincidentLattices(const ReciprocalLatticeDirection<dim>& rd,
                                                                                        const double& maxDen,
                                                                                        const int& N) const
    requires(dim == 3)
{
    ...
    std::transform(
        temp.begin(), temp.end(), std::back_inserter(output), [](const std::pair<IntScalarType, MatrixDimD>& p) { return p.second; });
    return output;
}

template<int dim>
std::vector<typename Lattice<dim>::MatrixDimD> Lattice<dim>::generateCoincidentLattices(const double& maxStrain,
                                                                                        const int& maxDen,
                                                                                        const int& N) const
    requires(dim == 2)
{
...
template<int dim>
std::vector<typename Lattice<dim>::MatrixDimD> Lattice<dim>::generateCoincidentLattices(const Lattice<dim>& undeformedLattice,
    ...
    for(const auto& pair1 : coPrimePairs) {
        ...
        if(abs(maxStrain) < epsilon) {
            ...
            for(const auto& alpha : alphaSequence.approximations) {
                ...
                double s1 = (q2ByAlpha.cartesian().squaredNorm() - latticeBasis.col(0).squaredNorm()) / latticeBasis.col(0).squaredNorm();
                ...
                for(const auto& pair2 : coPrimePairs) {
                    ...
                    for(const auto& beta : betaSequence.approximations) {
                        ...
                        double s2 =
                            (r2ByBeta.cartesian().squaredNorm() - latticeBasis.col(1).squaredNorm()) / latticeBasis.col(1).squaredNorm();
                        double s3 = (q2ByAlpha.cartesian().dot(r2ByBeta.cartesian()) - latticeBasis.col(0).dot(latticeBasis.col(1))) /
                        ...
                        if(numberOfConfigurations == maxConfigurations) return output;
                    }
                }
            }
        }
    ...
    requires(dim == 3)
{
    ...
    if(r0_temp.dot(boxVectors[0]) < 0) r0_temp = -1 * r0_temp;
    r1_temp = (boxVectors[2].cross(boxVectors[0])).reciprocalLatticeVector();
    if(r1_temp.dot(boxVectors[1]) < 0) r1_temp = -1 * r1_temp;
    r2_temp = (boxVectors[0].cross(boxVectors[1])).reciprocalLatticeVector();
    if(r2_temp.dot(boxVectors[2]) < 0) r2_temp = -1 * r2_temp;
    ...
    if(!filename.empty()) {
        ...
        for(const auto& vector : output) file << 1 << " " << vector.cartesian().transpose() << std::endl;
    ...
    requires(dim == 2)
{
    ...
    LatticeDirection<dim> v0((VectorDimI)boxVectors[0] / IntegerMath<IntScalarType>::gcd(boxVectors[0]), *this);
    ...
    int areaRatio = round(abs(boxLattice.latticeBasis.determinant() / (*this).latticeBasis.determinant()));
    ...
    for(int j = 0; j < scale1; ++j) {
        for(int k = 0; k < scale2; ++k) {
            ...
            int c1 = IntegerMath<IntScalarType>::positive_modulo(vector.dot(r1), boxVectors[1].dot(r1)) - vector.dot(r1);
            c1 = c1 / boxVectors[1].dot(r1);
            int c2 = IntegerMath<IntScalarType>::positive_modulo(vector.dot(r0), boxVectors[0].dot(r0)) - vector.dot(r0);
    ...
    if(!filename.empty()) {
        ...
        for(const auto& vector : output) file << 1 << " " << vector.cartesian().transpose() << " 0 " << std::endl;
