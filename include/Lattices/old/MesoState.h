...
public:
  ...
  explicit MesoState(const Gb<dim> &gb,
                     ...
                     const int &periodScaling, const double &a2,
                     const int &nImages);

  explicit MesoState(const ReferenceState<dim> &rS, const double &a2,
                     const int &nImages);

  double energy() const;
  Eigen::VectorXi
  getLocalStateCount(const int &numberOfInteractingPlanes) const {
