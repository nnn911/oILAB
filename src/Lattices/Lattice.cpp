        ...
        for(int i=0; i<scale0; ++i)
        {
            for(int j=0; j<scale1; ++j)
            {
                for(int k=0; k<scale2; ++k) {
                    ...
                    IntScalarType c0= IntegerMath<IntScalarType>::positive_modulo(vector.dot(r0),boxVectors[0].dot(r0)) -
                            vector.dot(r0);
                    c0= c0/boxVectors[0].dot(r0);
                    IntScalarType c1= IntegerMath<IntScalarType>::positive_modulo(vector.dot(r1),boxVectors[1].dot(r1)) -
                            vector.dot(r1);
                    c1= c1/boxVectors[1].dot(r1);
                    IntScalarType c2= IntegerMath<IntScalarType>::positive_modulo(vector.dot(r2),boxVectors[2].dot(r2)) -
                            vector.dot(r2);
